...
echo "Implementation,Processors,Insertions,Duration,Memory,LoadFactor,Locks,TblCnt,JunitTime,NUMABalancing,PageDefrag,Govenor,Misses"
...
for file in $1/*-tlc.txt; do
   ...
   MISSES=$(echo $FILE | grep -Po '\K[0-9.]* cache-misses' | cut -f1 -d " ")

   echo $IMPL,$PROCESSORS,$INSERTIONS,$DURATION,$(echo $MEM | sed s/,//g),$LOADFACTOR,$LOCKS,$TBLCNT,$(echo $JUNITTIME | sed s/,//g),$NUMA,$DEFRAG,$GOVENOR,$(echo $MISSES|sed s/\\.//g)
done
